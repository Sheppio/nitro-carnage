 ...
 */
export const DEFAULT_LINE = { lateral: 16, lateralPerMs: 0, braking: 16, accel: 6, topSpeed: 64, carTop: 51 };
 ...
 * same line.
 */
export function racingLine(track, opts = DEFAULT_LINE) {
    const cached = cache.get(track);
    if (cached && opts === DEFAULT_LINE)
        return cached;
    const n = track.n;
    ...
    const X = (i, o = offset[wrap(i)]) => px[wrap(i)] + nx[wrap(i)] * o;
    const Z = (i, o = offset[wrap(i)]) => pz[wrap(i)] + nz[wrap(i)] * o;
        ...
        for (let sweep = 0; sweep < SWEEPS; sweep++) {
            ...
            if (stride > 1) {
                for (let i = 0; i < n; i += stride) {
                    ...
                    for (let u = i + 1; u < end && u < n; u++)
                        offset[u] = oa + ((ob - oa) * (u - i)) / (j - i);
                }
            }
        }
    }
...
function signedCurvature(ax, az, bx, bz, cx, cz) {
...
//# sourceMappingURL=racingLine.js.map