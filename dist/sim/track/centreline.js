...
export function filletPolygon(corners, step = 0.25) {
    const n = corners.length;
    if (n < 3)
        throw new TrackGeometryError('a track needs at least three corners');
    const fillets = [];
    for (let i = 0; i < n; i++) {
 ...
 */
export function resample(dense, start) {
    ...
    const cum = [0];
    for (let k = 1; k < dx.length; k++)
        cum.push(cum[k - 1] + Math.hypot(dx[k] - dx[k - 1], dz[k] - dz[k - 1]));
...
//# sourceMappingURL=centreline.js.map