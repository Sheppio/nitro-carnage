...
export function free(ctx, x, z, r) {
    for (const [tx, tz, tr] of ctx.taken)
        if ((x - tx) * (x - tx) + (z - tz) * (z - tz) < (r + tr) * (r + tr))
            return false;
    return true;
}
/**
...
function besideRoad(ctx, r, clearance, water = false, rect) {
    ...
    for (let tries = 0; tries < 30; tries++) {
        ...
        if (ctx.roadDist(x, z) < keep)
            continue;
        if (!water && ctx.blocked(x, z, r))
            continue;
        if (!free(ctx, x, z, r))
            continue;
...
export function placeCountryside(ctx, rule) {
    const { rand } = ctx;
    switch (rule.kind) {
        case 'farms':
            for (let k = 0; k < rule.count; k++) {
                ...
                for (let s = 0; s < silos; s++)
                    part(ctx, 'silo', at, (15 + s * 7) * m, 9, 0, [6.3, 6.3, 18.2], rand());
                const bales = 3 + Math.floor(rand() * 5);
                for (let b = 0; b < bales; b++)
                    part(ctx, 'bale', at, (-13 + b * 1.8) * m, -12, Math.PI / 2, [1.6, 1.2, 1.6], rand());
                if (rand() < 0.6)
                    part(ctx, 'bales', at, -6 * m, -8, 0, [3.8, 2.4, 1.7], rand());
        ...
        case 'fields':
            for (let k = 0; k < rule.count; k++) {
                ...
                if (crop <= 1 && work < 0.5)
                    part(ctx, 'combine', at, (rand() - 0.5) * w * 0.6, (rand() - 0.5) * d * 0.5, rand() < 0.5 ? 0 : Math.PI, [7.2, 8.6, 5], rand());
                if (crop === 2 && work < 0.6)
                    part(ctx, 'tractor', at, (rand() - 0.5) * w * 0.6, (rand() - 0.5) * d * 0.6, 0, [2.1, 3.6, 2.9], rand());
                if (crop === 3) {
                    const n = 4 + Math.floor(rand() * 8);
                    for (let b = 0; b < n; b++)
                        part(ctx, 'bale', at, (rand() - 0.5) * (w - 4), (rand() - 0.5) * (d - 4), rand() * Math.PI, [1.6, 1.2, 1.6], rand());
                }
            }
            return true;
        case 'herds':
        ...
        case 'marina': {
            ...
            const px = (f, o) => [rule.from[0] + dx * len * f + nx * o, rule.from[1] + dz * len * f + nz * o];
...
export function pond(ctx, x, z, r) {
