...
export function createCar(x, z, yaw) {
    return {
 ...
 */
export function stepCar(car, intent, env, dt, stats = STOCK) {
...
function substep(car, intent, env, h, stats) {
    const fx = Math.sin(car.yaw), fz = Math.cos(car.yaw);
    const lx = fz, lz = -fx;
    const u = car.vx * fx + car.vz * fz;
