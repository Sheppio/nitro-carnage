...
export class Scenery {
     ...
     */
    constructor(track, theme, detail = 2) {
     ...
     */
    countryside(track, theme, detail) {
        ...
        const place = (x, z, rot, sx = 1, sy = 1, sz = 1, y = 0) => m.compose(new THREE.Vector3(x, y, z), q.setFromAxisAngle(up, rot), new THREE.Vector3(sx, sy, sz));
        const pick = (seed, n) => Math.min(n - 1, Math.floor(seed * n));
        const tall = (p, colour) => {
        ...
        for (const p of track.props) {
            if (small.has(p.kind)) {
                smallIndex++;
                if (detail <= 0 || (detail === 1 && smallIndex % 2 === 0))
                    continue;
            }
            const at = place(p.x, p.z, p.rot);
            switch (p.kind) {
                case 'farmhouse':
                    solid.add(model('farmhouse', pick(p.seed, 3)), at);
                    tall(p, 0xefe6d2);
                    break;
                case 'barn':
                    solid.add(model('barn', pick(p.seed, 3)), at);
                    tall(p, 0xa8322a);
                    break;
                case 'silo':
                    solid.add(model('silo', pick(p.seed, 3)), at);
                    tall(p, 0xc8ccd0);
                    break;
                case 'bale':
                    solid.add(model('bale', pick(p.seed, 3)), at);
                    break;
                case 'bales':
                    solid.add(model('bales', pick(p.seed, 2)), at);
                    break;
                case 'tractor':
                    solid.add(model('tractor', pick(p.seed, 3)), at);
                    break;
                case 'combine':
                    solid.add(model('combine', pick(p.seed, 2)), at);
                    tall(p, 0x3f8a3a);
                    break;
                case 'cow':
                    solid.add(model('cow', pick((p.seed % 0.5) * 2, 3) + (p.seed >= 0.5 ? 3 : 0)), at);
                    break;
                case 'sheep':
                    solid.add(model('sheep', p.seed >= 0.5 ? 1 : 0), at);
                    break;
                case 'windmill':
                ...
                case 'fence':
                    solid.add(fenceModel(p.w, p.d), at);
                    break;
                case 'flowers': {
                ...
                case 'reeds':
                    solid.add(model('reeds', pick(p.seed, 4)), at);
                    break;
                case 'warehouse':
                    solid.add(warehouseModel(p.w, p.d, p.h, pick(p.seed, 5)), at);
                    tall(p, 0x8a9aa8);
                    break;
                case 'forklift':
                    solid.add(model('forklift', pick(p.seed, 3)), at);
                    break;
                case 'flatbed':
                    solid.add(model('flatbed', pick(p.seed, 12)), at);
                    break;
                case 'pallets':
                    solid.add(model('pallets', pick(p.seed, 4)), at);
                    break;
                case 'crates':
                    solid.add(model('crates', pick(p.seed, 6)), at);
                    break;
                case 'drums':
                    solid.add(model('drums', pick(p.seed, 30)), at);
                    break;
                case 'ship':
                    solid.add(model('ship', pick(p.seed, 12)), at);
                    tall(p, 0x2a3a5a);
                    break;
                case 'yacht':
                    solid.add(model('yacht', pick(p.seed, 8)), at);
                    break;
                case 'pontoon':
                    solid.add(model('pontoon'), place(p.x, p.z, p.rot, p.w / 2, 1, p.d));
                    break;
                default: break;
            }
        }
