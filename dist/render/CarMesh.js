...
export class CarMesh {
    ...
    v = new THREE.Vector3();
    wheelScale = { front: new THREE.Vector3(1, 1, 1), rear: new THREE.Vector3(1, 1, 1), fr: WHEEL_R, rr: WHEEL_R };
     ...
     */
    condition(wrecked, ghost, time) {
        this.wrecked = wrecked;
        this.root.visible = !ghost || Math.sin(time * 40) > -0.3;
    }
    constructor(colour, shadowMaps, look = DEFAULT_LOOK) {
        this.colour = colour;
        this.look = look;
        this.root.name = 'car';
        ...
        this.shadow = new THREE.Mesh(new THREE.PlaneGeometry(2.8, 5.2), new THREE.MeshBasicMaterial({ map: blob(), transparent: true, depthWrite: false, opacity: shadowMaps ? 0.55 : 0.85 }));
    ...
    update(car, dt) {
        ...
        if (this.wrecked)
            this.body.rotation.set(0.12, 0, 0.3, 'YXZ');
        else
        ...
        this.shadow.material.opacity = Math.max(0.2, 0.8 - lift * 0.12);
    }
}
//# sourceMappingURL=CarMesh.js.map