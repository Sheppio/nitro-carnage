...
export class WeaponView {
     ...
     */
    update(armoury, time, cars) {
        let n = 0;
        for (const m of armoury.missiles) {
            if (m.done || time < m.t0 || time > m.end || n >= MAX_MISSILES)
                continue;
        ...
        if (this.mineLights.instanceColor)
            this.mineLights.instanceColor.needsUpdate = true;
    }
