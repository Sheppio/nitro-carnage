...
<!-- version -->**v0.1.13**<!-- /version --> — the build currently on Pages.
...
> **Milestone 4 of 8: weapons.** Front missiles, mines, and rear missiles once the
> mines run out. Cars take damage from shots and hard wall hits, smoke when they're
> hurt, and are wrecked at zero, returning a few seconds later with a third of their
> health. That works offline against bots and online in rooms of up to six (from M3),
> where a host's tab can close mid-race and the race carries on. Every screen works
> from the keyboard's arrow keys or a controller alone, on a Steam Deck, Xbox or
> PlayStation browser. Still to come: more tracks, hazards and audio (M5), car liveries
> and body styles (M6), and a track of the day, hotlaps and a no-weapons mode (M7). A
> shop and championship are deferred to M8, and may not happen. [`PLAN.md`](PLAN.md) has the whole design.
...
npm test           # 217 checks: simulation and networking (Node), and real browsers
```
...
217 checks across six suites. The browser suites swap the CDN for a local three.js and a
...
- **`sim.test.mjs`** (80, Node):
  ...
  - **Helpers:** interpolation, deadzones, framerate-independent smoothing, colour
    clash resolution.
- **`net.test.mjs`** (53, Node, an in-memory broker and a fake clock):
  - **Codecs:** round trips; worst-case sizes (car ≤ 54 bytes, heartbeat ≤ 174);
    truncation; wrapped time stamps.
  ...
  - **Races:** one grid and one GO everywhere; three clients and two bots to identical
    ...
    then races; a rammed car is shoved, once; the bandwidth budget.
...
- **`multiplayer.test.mjs`** (18, browser, up to four tabs): a room forms from a code
  and a share link; one host; colour clashes; only the host can start; a missile fired
...
- **`gamepad.test.mjs`** (15, browser, a virtual pad and nothing else): the lock prompt;
...
- **`keyboard.test.mjs`** (23, browser, keys and nothing else): on the menu, settings,
  join, lobby, pause and results screens, a walk with the arrow keys reaches every
  control. Also: the caret keeps left, right and Backspace in text fields; dropdowns
  change with left and right; Space toggles a switch; Esc goes back everywhere and
  resumes from pause without reopening it; in a race the arrows drive.
