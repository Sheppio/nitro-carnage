...
<!-- version -->**v0.1.11**<!-- /version --> — the build currently on Pages.
...
> **Milestone 3 of 7: multiplayer rooms.** Create a room, share its code or link, and
> race up to five friends, with bots to fill the grid. A host's tab can close mid-race
> and the race carries on; latecomers spectate until the next race. It plays from a
> controller alone, on a Steam Deck, Xbox or PlayStation browser. *Quick race* against
> bots and *Free drive* work offline. Weapons arrive in M4, and the shop and
> championship in M5; car liveries and body styles in M7. [`PLAN.md`](PLAN.md) has the whole design.
...
npm test           # 156 checks: simulation and networking (Node), and real browsers
```
...
Esc, the pad's Menu/Options or the ☰ button opens the pause menu. The weapons do
nothing until M4. The most recently used device drives the car, so picking up a
controller mid-race just works.
...
156 checks across five suites. The browser suites swap the CDN for a local three.js and a
...
- **`sim.test.mjs`** (62, Node):
  ...
  - **Helpers:** interpolation, deadzones, framerate-independent smoothing, colour
    clash resolution.
- **`net.test.mjs`** (40, Node, an in-memory broker and a fake clock):
...
- **`multiplayer.test.mjs`** (17, browser, up to four tabs): a room forms from a code
  and a share link; one host; colour clashes; only the host can start; a race to the
...
- **`gamepad.test.mjs`** (14, browser, a virtual pad and nothing else): the lock prompt;
  Xbox and PlayStation prompts; the on-screen keyboard; menu to race; RT drives;
  Menu/Options pauses, A resumes; leaving; the menu, lobby and HUD at 1280×800.
