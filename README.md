...
<!-- version -->**v0.1.14**<!-- /version --> — the build currently on Pages.
...
> **Milestone 5 of 8: more tracks, hazards and sound.** There are three circuits to
> choose from. **Neon Downtown** is the walled city grid. **Greenbelt** is fast parkland
> with grass verges, open lawns, a gravel stretch and a jump over the creek. **Tidewater
> Docks** is container canyons, a quay with nothing between you and the harbour, oil
> patches, and a level crossing whose train keeps its own timetable. Every engine, shot,
> explosion, bell and bar of music is synthesised in the browser. Weapons (M4) work
> offline against bots and online in rooms of up to six (M3). Every screen works from
> the arrow keys, a controller or a touchscreen. Still to come: car liveries and body
> styles (M6), and a track of the day, hotlaps and a no-weapons mode (M7). A shop and
> championship are deferred to M8, and may not happen. [`PLAN.md`](PLAN.md) has the whole design.
...
npm test           # 281 checks: simulation and networking (Node), and real browsers
```
...
281 checks across seven suites. The browser suites swap the CDN for a local three.js and a
...
- **`sim.test.mjs`** (126, Node):
  ...
  - **Helpers:** interpolation, deadzones, framerate-independent smoothing, colour
    clash resolution.
- **`net.test.mjs`** (54, Node, an in-memory broker and a fake clock):
...
- **`keyboard.test.mjs`** (24, browser, keys and nothing else): on the menu, settings,
