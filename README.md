...
> **Milestone 6 of 8: your car, your way.** Five body styles (Coupé, Hatch, Muscle,
> Wedge and Buggy), liveries, stripe and wheel colours, and a race number, chosen in the
> **Garage** and seen by everyone in the room. It's cosmetic only: every car drives the
> same. The three circuits (M5) are **Neon Downtown**, **Greenbelt** and **Tidewater
> Docks**, which has a level crossing whose train keeps its own timetable. Everything
> you hear is synthesised in the browser. There are weapons (M4) offline and online, in
> rooms of up to six (M3). Every screen works from the arrow keys, a controller or a
> touchscreen. Still to come: a track of the day, hotlaps and a no-weapons mode (M7). A
...
npm test           # 293 checks: simulation and networking (Node), and real browsers
```
...
293 checks across seven suites. The browser suites swap the CDN for a local three.js and a
...
- **`sim.test.mjs`** (126, Node):
  ...
  - **Helpers:** interpolation, deadzones, framerate-independent smoothing, colour
    clash resolution.
- **`net.test.mjs`** (58, Node, an in-memory broker and a fake clock):
