...
export interface TrainState {
  /** Distance along the rail of the front and the back of the train, metres. */
  head: number;
  tail: number;
  /** +1 running from the rail's `from` end to its `to` end, -1 back. */
  dir: 1 | -1;
  /** Speed along the rail, m/s (always positive; `dir` says which way). */
  speed: number;
  /** Which pass of the timetable this is. */
  pass: number;
}
 ...
 * comes back.
 */
export function trainAt(track: Track, raceTime: number): TrainState | null {
  const rw = track.def.railway;
  const rail = track.rail;
  if (!rw || !rail) return null;
  ...
  if (run > rail.length + len) return null; // gone: into the shed, or off the map
  const dir: 1 | -1 = pass % 2 === 0 ? 1 : -1;
  const head = dir > 0 ? run : rail.length - run;
  return { head, tail: head - dir * len, dir, speed: rw.speed, pass };
}

export const trainLength = (cars: number): number => cars * CAR_LENGTH + (cars - 1) * COUPLING;

/** The stretch of rail the road (with its verges) occupies. */
function crossingSpan(track: Track): [number, number] {
  const r = track.rail!;
...
export function crossingBlocked(track: Track, raceTime: number): boolean {
 ...
 */
export function crossingBusy(track: Track, t0: number, t1: number): boolean {
  for (let t = t0; t <= t1; t += 0.25) if (crossingBlocked(track, t)) return true;
  return crossingBlocked(track, t1);
}

/** Lights flashing: the train is on the crossing or due within `WARNING` seconds. */
export function crossingWarning(track: Track, raceTime: number): boolean {
 ...
 */
export function trainSegment(track: Track, tr: TrainState): { ax: number; az: number; bx: number; bz: number } | null {
  const r = track.rail!;
