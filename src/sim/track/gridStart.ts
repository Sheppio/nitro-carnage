...
export function gridBend(t: Track, s = 0): number {
 ...
 */
export function straightStart(t: Track, rebuild: (start: [number, number]) => Track): [number, number] | null {
  if (gridBend(t) <= 1 / GRID_RADIUS) return null;
  for (let s = 2; s < t.length; s += 2) {
    if (gridBend(t, s) > 1 / GRID_RADIUS) continue;
    // Whole metres move the line a fraction: check it where it lands.
    const p = t.poseAt(s);
    const start: [number, number] = [Math.round(p.x), Math.round(p.z)];
    if (gridBend(rebuild(start)) <= 1 / GRID_RADIUS) return start;
  }
  return null;
}
