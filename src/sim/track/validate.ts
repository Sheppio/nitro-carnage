...
import type { Track } from './buildTrack.js';
...
export function validateTrack(t: Track, opts: { minSeparation?: number } = {}): string | null {
  ...
  if (minR <= t.wallOffset + 1) return `corner radius ${minR.toFixed(1)} m`;
  ...
  for (let a = 0; a < t.wallCount; a++) {
    ...
    t.forWallsNear(Math.min(w[o]!, w[o + 2]!), Math.min(w[o + 1]!, w[o + 3]!), Math.max(w[o]!, w[o + 2]!), Math.max(w[o + 1]!, w[o + 3]!), (b) => {
      if (crossed || b <= a) return;
      const p = b * 6;
      // Neighbouring segments share an end; that is a joint, not a crossing.
      for (const [x, z] of [[w[o]!, w[o + 1]!], [w[o + 2]!, w[o + 3]!]] as const) {
        for (const [y, v] of [[w[p]!, w[p + 1]!], [w[p + 2]!, w[p + 3]!]] as const) {
          if (Math.abs(x - y) < 1e-9 && Math.abs(z - v) < 1e-9) return;
        }
      }
      if (closestSegSeg(w[o]!, w[o + 1]!, w[o + 2]!, w[o + 3]!, w[p]!, w[p + 1]!, w[p + 2]!, w[p + 3]!, tmp) < 1e-6) crossed = true;
    });
    if (crossed) return 'walls cross';
  }
  return null;
}
