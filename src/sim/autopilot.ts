...
export function autopilot(
  ...
  if (v < 1.5 && stopAt === null) st.stuck += dt;
  else st.stuck = 0;
  ...
  if (st.skill.top !== undefined && st.skill.top < 1) target = Math.min(target, SIM.car.topSpeed * st.skill.top);
  ...
  if (grip < 1) target *= Math.sqrt(grip);
  ...
  if (stopAt !== null) {
    ...
    if (toStop < 160) target = Math.min(target, Math.sqrt(2 * 10 * Math.max(0, toStop - 2)));
  }
  const err = target - v;
  if (target < 0.5 && v < 2) {
  ...
  if (slide > 0.28) out.throttle = 0;
  else if (slide > 0.16) out.throttle = Math.min(out.throttle, 0.4);
  ...
  if (st.skill.turbo && car.turbo > 1 && out.throttle > 0.9) {
    ...
    let straight = true;
    for (let a = 0; a < v * 2 + 20 && straight; a += 6) straight = line.speed[idx(p.s + a)]! >= SIM.car.topSpeed + 2;
    out.turbo = straight;
  }
  return out;
}
