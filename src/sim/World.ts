...
export interface Entrant {
   ...
   */
  remote: boolean;
  /** Health, 0-100. A remote car's comes from its owner's packets. */
  hp: number;
  ammo: { front: number; rear: number; mines: number };
  /** Seconds left burning after a wreck; 0 when the car is running. */
  wrecked: number;
  /** Times wrecked, and wrecks caused. */
  wrecks: number;
  kills: number;
  /** Counter for this car's shots and mines: with its id, each one's identity. */
  seq: number;
  /** Seconds until each weapon may fire again. */
  cooldown: { front: number; rear: number };
  /** Who last hurt this car, and when: a wall that finishes the job still counts as theirs. */
...
export type RaceEvent =
  ...
  /** A car fired a missile (`seq` is its shot counter). From the car's centre and heading. */
  | { kind: 'fire'; id: string; seq: number; weapon: MissileKind; x: number; z: number; yaw: number; time: number }
  /** A car dropped a mine, placed at (x, z). */
  | { kind: 'mine'; id: string; seq: number; x: number; z: number; time: number }
  /**
   * A shot or a mine found a car. `by`/`seq` name the shot (or mine). With a
   * remote victim nothing was applied here: its owner applies the damage.
   */
  | { kind: 'hit'; id: string; by: string; seq: number; weapon: WeaponKind; damage: number; x: number; z: number; remote: boolean }
  /** A missile stopped at a wall. */
...
export class World {
  ...
  rivalsOf(id: string): Rival[] {
    ...
    for (const e of this.entrants) {
      if (e.id === id || e.ghost > 0 || e.wrecked > 0) continue;
  ...
  stopLine(e: Entrant): number | null {
    ...
    if (this.track.deltaS(e.s, rail.s - 6) < 0 || dist > 160) return null;
   ...
   */
  advanceTo(target: number, maxDt = 1): number {
  ...
  private fire(e: Entrant, t: number): void {
    const c = e.car;
  ...
  private launch(e: Entrant, kind: MissileKind, t: number): void {
    const c = e.car;
    const seq = ++e.seq;
   ...
   */
  private stepWeapons(t0: number, t1: number): void {
    const arm = this.armoury;
    if (!arm.missiles.length && !arm.mines.length) return;
    ...
    for (const mine of arm.mines) {
      ...
      if (t1 < mine.armAt) continue;
      for (const e of this.entrants) {
        if (e.remote) continue;
        const target = targets.find((x) => x.id === e.id)!;
        if (!Armoury.onMine(mine, target)) continue;
        mine.done = true;
   ...
   * shots made.
   */
  hit(victim: string, by: string, seq: number, weapon: WeaponKind, damage: number, x: number, z: number): void {
   ...
   */
  damage(e: Entrant, amount: number, by: string | null, attack: boolean): void {
    if (e.remote || e.wrecked > 0 || e.lap.finished || amount <= 0) return;
    ...
    if (e.hp <= 0) this.wreck(e, by === e.id ? null : by);
  }

  private wreck(e: Entrant, by: string | null): void {
    e.hp = 0;
   ...
   */
  creditWreck(victim: string, by: string | null): void {
    const e = this.entrants.find((x) => x.id === victim);
    if (e) e.wrecks++;
    const killer = by ? this.entrants.find((x) => x.id === by) : undefined;
    if (killer) killer.kills++;
  ...
  private collideCars(): void {
    ...
    for (let i = 0; i < list.length; i++) {
      const a = list[i]!;
      if (a.ghost > 0 || a.wrecked > 0) continue;
      for (let j = i + 1; j < list.length; j++) {
        const b = list[j]!;
        if (b.ghost > 0 || b.wrecked > 0) continue;
