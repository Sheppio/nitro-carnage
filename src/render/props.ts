...
function wheel(b: MeshBuilder, x: number, y: number, z: number, r: number, w: number, hub = 0x8a8c90): void {
...
function gable(b: MeshBuilder, y: number, w: number, d: number, rise: number, hex: number): void {
 ...
 */
function hull(b: MeshBuilder, beam: number, len: number, y0: number, y1: number, side: number, deck: number, bow = 0.18): void {
  const hb = beam / 2, hl = len / 2, zb = hl - len * bow;
  b.prism([[-hb, -hl], [-hb, zb], [0, hl], [hb, zb], [hb, -hl]], y0, y1, side, { top: deck });
}

type Build = (b: MeshBuilder, v: number) => void;
...
const MODELS: Record<string, Build> = {
  ...
  farmhouse(b, v) {
    ...
    for (const z of [-3.2, 3.2]) for (const y of [1.8, 4.3]) b.box(4.53, y, z, 0.08, 1.2, 1.1, GLASS);
    for (const z of [-3.2, 0, 3.2]) b.box(-4.53, 4.3, z, 0.08, 1.2, 1.1, GLASS);
  ...
  silo(b, v) {
    ...
    for (const y of [4, 8, 12]) b.cylinder(0, y, 0, 3.08, 3.08, 0.25, 12, 0x8a8e94, { skipBottom: true });
  ...
  bales(b, v) {
    ...
    for (let row = 0; row < 3; row++) {
      for (let k = 0; k < 3 - row; k++) b.box((k - (2 - row) / 2) * 1.25, 0.28 + row * 0.56, 0, 1.2, 0.55, 2.4, straw, { top: 0xe8cc78 });
    }
  },
  ...
  cow(b, v) {
    ...
    for (const [x, z] of [[0.24, 0.62], [-0.24, 0.62], [0.24, -0.62], [-0.24, -0.62]] as const) b.box(x, 0.36, z, 0.16, 0.72, 0.16, coat, { skipBottom: true });
  },
  ...
  sheep(b, v) {
    ...
    for (const [x, z] of [[0.2, 0.34], [-0.2, 0.34], [0.2, -0.34], [-0.2, -0.34]] as const) b.box(x, 0.24, z, 0.11, 0.48, 0.11, 0x2a2826, { skipBottom: true });
  },
  ...
  forklift(b, v) {
    ...
    for (const [x, z] of [[0.52, 0.4], [-0.52, 0.4], [0.52, -0.8], [-0.52, -0.8]] as const) b.box(x, 1.75, z, 0.07, 0.9, 0.07, 0x2a2a2a);
  ...
  flatbed(b, v) {
    ...
    if (load === 1) b.box(0, 2.6, -1, 2.4, 2.6, 6.1, [0xb8412f, 0x2f6fa8, 0x3f8a5a][v % 3]!, { skipBottom: true, sides: 0x8a8a8a });
    if (load === 2) for (const z of [-3.5, -1.5, 0.5]) b.box(0, 1.8, z, 1.8, 1.1, 1.6, 0xa87a48, { skipBottom: true, top: 0xc0945e });
    for (const z of [4, -2.5, -4]) for (const x of [1.15, -1.15]) wheel(b, x, 0.5, z, 0.5, 0.35);
  },

  /** A stack of 1-4 pallets. */
  pallets(b, v) {
    const n = 1 + (v % 4);
    for (let k = 0; k < n; k++) b.box(0, 0.075 + k * 0.15, 0, 1.2, 0.14, 1.0, 0xb08a58, { skipBottom: true, sides: 0x7a5a38 });
  },
  ...
  crates(b, v) {
    ...
    if (v >= 3) b.at(0.1, s, 0.05, 0.4, 0, (w) => w.box(0, 0.45, 0, 0.9, 0.9, 0.9, wood, { skipBottom: true, top: 0xc0945e }));
  },
...
export function model(kind: string, variant = 0): THREE.BufferGeometry {
  const key = `${kind}:${variant}`;
  let g = cache.get(key);
  if (!g) {
    const build = MODELS[kind];
    if (!build) throw new Error(`no model "${kind}"`);
...
export function sailsModel(): THREE.BufferGeometry {
  const b = new MeshBuilder();
  for (let k = 0; k < 4; k++) {
 ...
 */
export function fieldModel(w: number, d: number, crop: number): { ground: THREE.BufferGeometry; standing: THREE.BufferGeometry | null } {
  ...
  for (let k = 0; k < rows; k++) {
    ...
    if (crop === 0) s.box(x, 1.1, 0, 0.9, 2.2, d - 1.2, k % 2 ? 0x5a8a2e : 0x4f7e2a, { skipBottom: true, top: 0x9aa040 });
    else if (crop === 1) g.box(x, 0.05, 0, 0.8, 0.06, d, k % 2 ? 0xd9b44a : 0xc49a36, { skipBottom: true });
    else if (crop === 2) s.box(x, 0.12, 0, 0.7, 0.24, d - 0.4, 0x7a5638, { skipBottom: true, insetX: 0.25 });
    else g.box(x, 0.05, 0, 0.8, 0.06, d, k % 2 ? 0xd0bc78 : 0xa89050, { skipBottom: true });
  }
...
export function flowerBedModel(len: number, seed: number): { ground: THREE.BufferGeometry; standing: THREE.BufferGeometry } {
...
export function fenceModel(w: number, d: number): THREE.BufferGeometry {
  const b = new MeshBuilder();
  const wood = 0x8a6a48;
  const side = (x0: number, z0: number, x1: number, z1: number, gate: boolean): void => {
    const len = Math.hypot(x1 - x0, z1 - z0);
    const yaw = Math.atan2(x1 - x0, z1 - z0);
    b.at(x0, 0, z0, yaw, 0, (f) => {
      const posts = Math.max(1, Math.round(len / 3));
      for (let k = 0; k <= posts; k++) f.box(0, 0.6, (k * len) / posts, 0.14, 1.2, 0.14, wood, { skipBottom: true });
      const railLen = gate ? len / 2 - 2 : len;
      for (const y of [0.55, 1.0]) {
        f.box(0, y, railLen / 2, 0.06, 0.08, railLen, wood);
        if (gate) f.box(0, y, len - railLen / 2, 0.06, 0.08, railLen, wood);
      }
    });
  };
  const hw = w / 2, hd = d / 2;
...
export function warehouseModel(w: number, d: number, h: number, v: number): THREE.BufferGeometry {
  const b = new MeshBuilder();
  ...
  for (let x = -w / 2 + 1.5; x < w / 2; x += 3) {
    for (const z of [d / 2 + 0.06, -d / 2 - 0.06]) b.box(x, h / 2, z, 0.35, h - 0.3, 0.12, rib);
  }
...
export function pondModel(r: number): { bank: THREE.BufferGeometry; water: THREE.BufferGeometry } {
