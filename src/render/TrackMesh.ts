...
export function buildTrackMesh(track: Track, theme: Theme): THREE.Group {
  ...
  const dirtAt = (i: number): boolean => track.def.surfaces.some((z) => {
    if (z.shape !== 'span' || z.surface !== Surface.Dirt) return false;
  ...
  if (rail) {
    ...
    const pt = (u: number, side: number, y: number) => ({ x: rail.ax + rail.dx * u + px * side, y, z: rail.az + rail.dz * u + pz * side });
