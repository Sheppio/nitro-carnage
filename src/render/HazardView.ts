...
export class HazardView {
  ...
  constructor(private track: Track) {
    ...
    for (let k = 0; k < rw.cars; k++) this.wagons.setColorAt(k, c.setHex(k === 0 ? 0xc8332a : [0x3a5a8a, 0x5a6a4a, 0x8a6a3a][k % 3]!));
    this.wagons.count = 0;
    ...
    for (const side of [-1, 1]) {
      ...
      const arm = new THREE.Mesh(
        new MeshBuilder().box(side * track.halfWidth * 0.5 + side * 0.4, 0, 0, track.halfWidth + 0.8, 0.18, 0.18, 0xf2f2f2).build(),
        flatMaterial(),
      );
   ...
   */
  update(raceTime: number, dt: number): void {
    const rail = this.track.rail;
    if (!rail || !this.wagons) return;
    ...
    if (tr) {
      ...
      for (let k = 0; k < this.cars; k++) {
        ...
        if (u < CAR_LENGTH / 2 || u > rail.length - CAR_LENGTH / 2) continue;
    ...
    if (this.wagons.instanceColor) this.wagons.instanceColor.needsUpdate = true;
    ...
    this.lights.forEach((l, i) => (l.material as THREE.MeshBasicMaterial).color.setHex(warn && i % 2 === phase ? 0xff2a1a : 0x3a0a0a));
    this.lowered += ((warn ? 1 : 0) - this.lowered) * Math.min(1, dt * 2.5);
    // Raised, a boom stands upright: towards +Y from whichever way it reaches.
    for (const b of this.booms) b.rotation.z = (1 - this.lowered) * (Math.PI / 2) * (b.userData.side as number);
  }
}
