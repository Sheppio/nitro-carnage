...
export type QualityId = 'potato' | 'low' | 'medium' | 'high';

export interface QualityPreset {
  label: string;
  pixelRatioCap: number;
  /** 0 disables shadow mapping entirely (a blob shadow stays under each car). */
  shadowMapSize: number;
  softShadows: boolean;
  particles: number;
  tyreMarks: number;
  /** Fog end and camera far plane, metres. */
  drawDistance: number;
  antialias: boolean;
  /** Small scenery clutter (M10): 2 all of it, 1 every other piece, 0 none. */
