...
import type { DriveSample, InputSource, SchemeId } from './sources.js';
...
export class InputManager {
  ...
  read(dt: number): DriveIntent {
    const now = performance.now();
    let chosen: DriveSample | null = null;
  ...
  rumble(weak: number, strong: number, ms: number): void {
    ...
    if (this.gamepad.connected) this.gamepad.triggerRumble(weak, strong, ms);
    if (this.scheme === 'touch') navigator.vibrate?.(Math.round(ms));
  }
