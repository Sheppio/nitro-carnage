...
import type { Heartbeat } from './codec.js';
...
export interface PeerRecord {
  id: PlayerId;
  name: string;
  /** Palette id this peer asked for; `resolvedColours` settles clashes. */
  colour: string;
  claimsHost: boolean;
  ready: boolean;
  ver: string;
...
export interface RoomEvents extends Record<string, unknown> {
  ...
  /** Fired whenever authority moves — including the initial claim. */
  hostChange: { hostId: PlayerId | null; isHost: boolean; reason: 'initial' | 'election' | 'yield' };
  /** Another client's heartbeat: the room and race state it carries. */
  heartbeat: { hb: Heartbeat };
  /** This client arrived at a room that already holds six players. */
  roomFull: { capacity: number };
}

/** What the host's heartbeat carries besides its own identity. */
...
export class RoomSession {
  ...
  stateProvider: () => RoomState = () => LOBBY_STATE;
  ...
  setIdentity(name: string, colour: string = this.colour, look: string = this.look): void {
    if (name === this.name && colour === this.colour && look === this.look) return;
    this.name = name;
    this.colour = colour;
    this.look = look;
  ...
  join(): void {
    ...
    this.unsubs.push(
      this.net.subscribe(Topics.presenceAll(this.roomId), (topic, payload) => this.onPresence(segment(topic, 0), payload)),
      this.net.subscribe(Topics.heartbeat(this.roomId), (_t, payload) => this.onHeartbeat(payload)),
    );

    this.announcePresence();
    this.timers.push(
      this.clock.setInterval(() => this.announcePresence(), NET.presenceMs),
      this.clock.setInterval(() => this.tick(), 250),
      this.clock.setInterval(() => this.beat(), 1000 / NET.heartbeatHz),
    );
  ...
  leave(): void {
    ...
    for (const t of this.timers) this.clock.clearInterval(t);
    this.timers = [];
  ...
  private presencePayload(alive: boolean): string {
  ...
  private onPresence(id: PlayerId, payload: string): void {
    ...
    if (changed) this.events.emit('roster', { peers: [...this.peers.values()] });
  ...
  private onHeartbeat(payload: string): void {
    ...
    if (!hb || hb.hostId === this.playerId) return;
    ...
    if (!this._isHost) this.events.emit('heartbeat', { hb });
  }

  private beat(): void {
    if (!this._isHost || !this.joined) return;
    this.net.publish(
      Topics.heartbeat(this.roomId),
      encodeHeartbeat({ hostId: this.playerId, seq: ++this.beatSeq, roomT: this.roomNow(), ...this.stateProvider() }),
    );
  }
