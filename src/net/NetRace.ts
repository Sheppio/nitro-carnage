...
export interface NetCarInfo {
  id: string;
  name: string;
  /** Palette id, after clashes are resolved. */
  colour: string;
  bot: boolean;
  you: boolean;
}
...
export interface NetResultRow {
  position: number;
  id: string;
  /** Race time from GO, seconds; null for a car that did not finish. */
  time: number | null;
  laps: number;
}

export interface NetRaceEvents extends Record<string, unknown> {
  /** The room or race state changed: phase, grid, settings, roster. */
  state: { state: RoomState };
  /** A race world exists: build the view. */
  raceStart: { world: World; spectating: boolean };
  /** The race world is gone (back to the lobby). */
  raceEnd: Record<string, never>;
  results: { rows: NetResultRow[] };
  roster: { peers: PeerRecord[] };
  hostChange: { isHost: boolean };
  roomFull: { capacity: number };
  /** World events for the HUD (laps, finishes, respawns, bumps). */
  race: { ev: RaceEvent };
}
...
export class NetRace {
  ...
  roomAt(worldTime: number): number {
  ...
  private apply(next: RoomState): void {
    const prevPhase = this.state.phase;
    this.state = next;
    const racing = next.phase !== 'L' && next.goAt > 0 && next.grid.length > 0;
    if (racing && next.goAt !== this.raceGoAt) this.beginRace(next);
    if (!racing && this.world) this.endRace();
    ...
    if (prevPhase !== next.phase || !racing) this.events.emit('state', { state: next });
  }
