 ...
 */
export function carIcon(look: CarLook, colourId: string, width = 44, height = 22): HTMLCanvasElement {
  ...
  if (look.body === 'buggy') {
    g.fillStyle = '#16171b';
    for (const [fx, fy] of [[0.18, 0], [0.18, 1], [0.8, 0], [0.8, 1]]) g.fillRect(x0 + L * fx! - 3, y0 + (W - 2) * fy! - 2, 6, 4);
  }
  // Cabin glass.
  ...
  if (look.pattern === 'twin') {
    ...
    for (let i = 0; i < 3; i++) for (let j = 0; j < 4; j++) if ((i + j) % 2) g.fillRect(x0 + L * 0.72 + i * (L * 0.08), y0 + j * (W / 4), L * 0.08, W / 4);
  }
  return c;
}
